...
import { Application, AndroidApplication, AndroidActivityResultEventData, Utils } from '@nativescript/core';

const GOOGLE_FIT_PERMISSIONS_REQUEST_CODE = 2;

declare const android: any, com: any, global: any;
...
export class HealthData extends Common implements HealthDataApi {
	...
	requestAuthorization(types: Array<HealthDataType>): Promise<boolean> {
		return Promise.all([this.requestHardwarePermissions(), this.requestAuthorizationInternal(types)]).then((results) =>
			Promise.resolve(results[0] && results[1])
		);
	}
	...
	requestAuthorizationInternal(types: Array<HealthDataType>): Promise<boolean> {
		return new Promise<boolean>((resolve, reject) => {
			...
			const activityResultHandler = (args: AndroidActivityResultEventData) => {
				...
				resolve(
					args.requestCode === GOOGLE_FIT_PERMISSIONS_REQUEST_CODE && args.resultCode === android.app.Activity.RESULT_OK
				);
			};
					...
					Fitness.getHistoryClient(
						...
						.addOnSuccessListener(
							new com.google.android.gms.tasks.OnSuccessListener({
								onSuccess: (
									dataReadResponse: any /* com.google.android.gms.fitness.result.DataReadResponse */
								) => {
        ...
        */
				...
				if (packageName) {
					if (!packageToAppNameCache.has(packageName)) {
						try {
							const appName = packageManager.getApplicationLabel(
