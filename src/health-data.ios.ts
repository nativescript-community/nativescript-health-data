...
export class HealthData extends Common implements HealthDataApi {
	...
	isAuthorized(types: Array<HealthDataType>): Promise<boolean> {
		return new Promise<boolean>((resolve, reject) => {
			...
			types.forEach((t) => {
				authorized =
					authorized &&
					this.healthStore.authorizationStatusForType(this.resolveDataType(acceptableDataTypes[t.name])) ===
	...
	requestAuthorization(types: Array<HealthDataType>): Promise<boolean> {
		return new Promise<boolean>((resolve, reject) => {
			...
			types
				.filter((t) => t.accessType === 'read' || t.accessType === 'readAndWrite')
				.forEach((t) => readDataTypes.addObject(this.resolveDataType(acceptableDataTypes[t.name])));
			types
				.filter((t) => t.accessType === 'write' || t.accessType === 'readAndWrite')
	...
	private queryForQuantityOrCategoryData(
		dataType: string,
		opts: QueryRequest,
		callback: (data: Array<ResponseItem>, error: string) => void
	) {
		...
		let query = HKSampleQuery.alloc().initWithSampleTypePredicateLimitSortDescriptorsResultsHandler(
			...
			null,
			sortBy,
			(query: HKSampleQuery, listResults: NSArray<HKSample>, error: NSError) => {
				if (listResults) {
	...
	private queryForStatisticsCollectionData(
		dataType: string,
		opts: QueryRequest,
		callback: (data: Array<ResponseItem>, error: string) => void
	) {
		...
		const query =
			HKStatisticsCollectionQuery.alloc().initWithQuantityTypeQuantitySamplePredicateOptionsAnchorDateIntervalComponents(
				objectType,
				null,
				16,
				opts.startDate,
		...
		query.initialResultsHandler = function (
			query: HKStatisticsCollectionQuery,
			listResults: HKStatisticsCollection,
			error: NSError
		) {
	...
	private monitorData(dataType: string, opts: StartMonitoringRequest): void {
		...
		let query = HKObserverQuery.alloc().initWithSampleTypePredicateUpdateHandler(
			objectType as HKSampleType,
			null,
			(observerQuery: HKObserverQuery, handler: () => void, error: NSError) => {
				if (error) {
		...
		if (opts.enableBackgroundUpdates) {
			this.healthStore.enableBackgroundDeliveryForTypeFrequencyWithCompletion(
				objectType,
				this.getHKUpdateFrequency(opts.backgroundUpdateFrequency),
				(success: boolean, error: NSError) => {
					if (!success) {
						opts.onError(error.localizedDescription);
					}
